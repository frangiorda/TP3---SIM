...
def simular():
    ...
    for dia in range(1, N + 1):
        if fase == 1:
            ...
            capacidad = descargas[bisect.bisect_left(descargas_acum, rnd_desc)]
        else:
            rnd_lleg = round(float(np.random.random()), 2)
            rnd_desc = round(float(np.random.uniform(0, 1)), 2)
            # λ = 20 barcos cada 10 horas = 2 barcos por hora → λ = 48
            llegadas_dia = int(np.random.poisson(48))
            base = int(np.random.choice(descargas, p=prob_descargas))
            extra =   int(0 + rnd_desc * 9) # distribución uniforme entre 0 y 9
            capacidad = extra
